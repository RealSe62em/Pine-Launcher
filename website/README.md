...
Download links target the latest GitHub Release. At runtime, `script.js` reads the latest release version, exact Windows x64, Windows ARM64, Debian amd64, and Arch Linux x64 asset URLs and sizes, plus the GitHub-provided Windows x64 SHA-256 digest from the Releases API. The values in `index.html` are 1.2.0 fallbacks for browsers or networks that block that API.
