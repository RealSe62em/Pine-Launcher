...
function normalizeActivityButtons(buttons) {
  if (!Array.isArray(buttons)) return undefined;
  const normalized = buttons.flatMap(button => {
    const label = activityText(button?.label).slice(0, 32);
    const url = normalizeActivityUrl(button?.url);
    return label ? [{ label, url }] : [];
  }).filter(button => button.url).slice(0, 2);
...
function readSavedServersStrict(instanceDir, filename = 'servers.dat') {
  const file = fs.readFileSync(path.join(instanceDir, filename));
  const decoded = file[0] === 0x1f && file[1] === 0x8b ? zlib.gunzipSync(file) : file;
  const root = readNbt(decoded);
  if (!Array.isArray(root?.servers)) throw new Error('servers.dat does not contain a server list');
  return root.servers.filter(item => item && typeof item.name === 'string' && typeof item.ip === 'string');
}

function readSavedServers(instanceDir) {
  try { return readSavedServersStrict(instanceDir); }
  catch { return []; }
}
...
function discordIpcEndpoints({ platform = process.platform, pipePrefix, maxPipeIndex = 9 } = {}) {
  if (pipePrefix) return Array.from({ length: maxPipeIndex + 1 }, (_, index) => `${pipePrefix}${index}`);
  if (platform === 'win32') {
    return Array.from({ length: maxPipeIndex + 1 }, (_, index) => `\\\\?\\pipe\\discord-ipc-${index}`);
  }
  // Use POSIX joining for Linux endpoints even when this function is being
  // validated from a Windows build host. Node's global path implementation
  // follows the host OS, not the requested target platform.
  ...
  const folders = [...new Set(roots.flatMap(root => clientFolders.map(folder => folder
    ? linuxPath.join(root, ...folder.split('/'))
    : root)))];
  return folders.flatMap(root => Array.from({ length: maxPipeIndex + 1 }, (_, index) => linuxPath.join(root, `discord-ipc-${index}`)));
}

class DiscordPresence {
  constructor(applicationId, { logger = () => {}, pipePrefix, maxPipeIndex = 9, platform = process.platform } = {}) {
    this.applicationId = String(applicationId || '');
    this.logger = logger;
    this.endpoints = discordIpcEndpoints({ platform, pipePrefix, maxPipeIndex });
    this.enabled = false;
    this.socket = null;
    this.ready = false;
  ...
  tryPipe(index) {
    if (!this.enabled || index >= this.endpoints.length) return this.scheduleReconnect();
    const socket = net.createConnection(this.endpoints[index]);
    let settled = false;
    socket.once('connect', () => {
      settled = true;
      this.socket = socket;
  ...
  setActivity(activity) {
    this.pendingActivity = activity ? {
      ...
      details_url: normalizeActivityUrl(activity.detailsUrl),
      state: activityText(activity.state),
      state_url: normalizeActivityUrl(activity.stateUrl),
      ...
      assets: activity.largeImageKey ? {
        ...
        large_url: normalizeActivityUrl(activity.largeImageUrl),
      } : undefined,
      buttons: normalizeActivityButtons(activity.buttons),
    } : null;
    this.flush();
  }
