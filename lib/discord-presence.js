...
function discordIpcEndpoints({ platform = process.platform, pipePrefix, maxPipeIndex = 9 } = {}) {
  ...
  const folders = [...new Set(roots.flatMap(root => [root, path.join(root, 'app', 'com.discordapp.Discord')]))];
  return folders.flatMap(root => Array.from({ length: maxPipeIndex + 1 }, (_, index) => path.join(root, `discord-ipc-${index}`)));
}
