...
function buildTransferPlan({ source, sourceFormat = 'folder', sourceLauncher = 'Minecraft', selection = {}, defaults = {}, warnings = [], rejected = [] }) {
  ...
  function visit(current, relative = '') {
    ...
    for (const entry of entries) {
      ...
      if (top === '.pine-content-installs') {
        safeRejected.push({ path: childRelative, reason: 'Pending content transactions are not imported' });
        continue;
      }
  ...
  const fingerprint = crypto.createHash('sha256').update(JSON.stringify({ source: root, selection: selected, files: files.map(file => [file.path, file.size, Math.trunc(file.mtimeMs)]) })).digest('hex');
...
function createTransferInclude(selection = {}, defaults = {}) {
  const selected = normalizedSelection(selection, defaults);
  return relative => selected[categoryFor(relative)] !== false && portableRelative(relative).split('/')[0].toLowerCase() !== '.pine-content-installs' && !PRIVATE_ROOTS.has(portableRelative(relative).split('/')[0].toLowerCase());
}
