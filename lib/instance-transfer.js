...
function inspectTree(source, { include = () => true } = {}) {
...
async function copyInstanceTransactional({ source, destination, onProgress = () => {}, include = () => true, signal = null }) {
  throwIfAborted(signal);
  const target = path.resolve(destination);
  const parent = path.dirname(target);
  const plan = inspectTree(source, { include });
...
module.exports = { abortError, availableBytes, copyInstanceTransactional, createDuplicationFilter, inspectTree, throwIfAborted, validateCopiedTree };
