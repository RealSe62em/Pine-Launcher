...
function createDuplicationFilter(components = null) {
  if (!components || typeof components !== 'object') components = {};
  ...
  ['logs', 'crash-reports', 'quickplay', '.pine-activity.json', '.pine-content-installs'].forEach(root => excluded.add(root));
  return relative => {
...
function createMigrationFilter() {
  const includeDuplicate = createDuplicationFilter();
  // These folders are generated for one exact Minecraft/loader combination.
  // The migrated instance rebuilds them on first launch while retaining every
  // user-owned file: worlds, mods, configs, servers, packs and screenshots.
  const generated = new Set(['versions', 'libraries', 'assets', 'natives', 'installers', '.fabric']);
  return relative => {
    const top = String(relative || '').split(/[\\/]/, 1)[0].toLowerCase();
    return includeDuplicate(relative) && !generated.has(top);
  };
}
...
module.exports = { abortError, availableBytes, copyInstanceTransactional, createDuplicationFilter, createMigrationFilter, fileSha256, inspectTree, throwIfAborted, validateCopiedTree };
