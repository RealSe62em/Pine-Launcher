...
const RULES = [
  ...
  { id: 'duplicate-mod', confidence: 'high', title: 'Duplicate mod IDs were detected', pattern: /duplicate\s+mods?\b|duplicate[^\r\n]*\bmod\s+ids?\b|ModResolutionException[^\r\n]*duplicate|Found duplicate mods\b/i, explanation: 'Two installed files appear to provide the same mod identity.', actions: ['Disable the older duplicate mod file', 'Open Content and remove the duplicate before retrying'] },
...
function redactSensitiveLog(value, limit = 10 * 1024 * 1024) {
  return String(value || '')
    ...
    .replace(/\b(access_?token|refresh_?token|authorization|client_?token|xsts_?token|identityToken)\b[\"']?\s*[=: ]\s*[\"']?(?:Bearer\s+)?[^\s,;\"']+/gi, '$1=[redacted]')
...
function suspectFromLog(text, mods = []) {
  // Dependency-resolution failures name both the dependent mod and its library,
  // but neither is evidence of a crashing implementation. Fabric's resolution
  // message should be reported as a dependency problem instead.
  if (/HARD_DEP(?:_NO_CANDIDATE)?|requires.*(?:which is missing|not installed)|missing (?:mandatory )?dependenc|Mod resolution failed/i.test(text)) return null;
  const enabled = (Array.isArray(mods) ? mods : []).filter(mod => !mod.disabled && mod.filename);
  const lines = String(text || '').split(/\r?\n/);
  const failureLine = /Suspected Mod:|Failure message:|Mod File:|Caused by:|(?:Exception|Error)(?::|\b)|MixinApplyError|MixinTransformerError|InvalidMixinException|InjectionError/i;
  let best = null;
  for (const mod of enabled) {
    ...
    for (const line of lines) {
      ...
      for (const name of names) {
        ...
        if (!new RegExp(`(^|[^a-z0-9_.-])${escaped}([^a-z0-9_.-]|$)`, 'i').test(line)) continue;
        const explicit = /Suspected Mod:|Failure message:|Mod File:/i.test(line);
        score = Math.max(score, explicit ? 5 : 3);
        evidence.push(line);
      }
    }
    if (score && (!best || score > best.score)) best = { ...mod, score, evidence };
  }
  if (!best || best.score < 3) return null;
  return {
    filename: best.filename,
    title: best.title || best.filename.replace(/\.jar$/i, ''),
    projectId: best.projectId || null,
    evidence: best.evidence.slice(0, 3).join('\n'),
  };
}

function fabricDependencyFinding(text) {
  const match = text.match(/Mod ['‘]([^'’]+)['’]\s*\(([^)]+)\)[^\n]*?requires\s+(?:version\s+([^\s]+)(?:\s+or later)?\s+of\s+)?([a-z0-9_.-]+),\s+which is missing/iu);
  if (!match) return null;
  const [, dependentTitle, dependentId, version, dependencyId] = match;
  const versionText = version ? ` ${version}${/or later/i.test(match[0]) ? ' or later' : ''}` : '';
  return {
    id: 'dependency',
    confidence: 'high',
    title: `${dependentTitle} needs ${dependencyId}`,
    explanation: `${dependentTitle} (${dependentId}) cannot load because ${dependencyId}${versionText} is missing or disabled. The dependency is not the crashing mod.`,
    actions: [`Enable or install ${dependencyId}${versionText}`, `Keep ${dependentTitle} enabled after its dependency is available`],
...
function diagnoseCrash(log, context = {}) {
  ...
  const findings = [];
  const dependencyFinding = fabricDependencyFinding(text);
  for (const rule of RULES) {
    const index = lines.findIndex(line => rule.pattern.test(line));
    if (index < 0) continue;
    findings.push(rule.id === 'dependency' && dependencyFinding
      ? dependencyFinding
      : { id: rule.id, confidence: rule.confidence, title: rule.title, explanation: rule.explanation, actions: rule.actions, evidence: cleanEvidence(lines.slice(Math.max(0, index - 2), index + 3).join('\n')) });
  }
  const suspectMod = suspectFromLog(text, context.mods);
  if (suspectMod) {
    findings.unshift({
      id: 'suspected-mod', confidence: 'medium', title: `The crash report names ${suspectMod.title}`,
      explanation: 'An explicit exception or crash-attribution line names this mod. Pine cannot prove causation, so disabling it temporarily is safer than deleting it.',
      actions: ['Disable this mod and retry', 'Restore it from Content if the crash continues'],
      evidence: cleanEvidence(suspectMod.evidence),
    });
  }
  return {
