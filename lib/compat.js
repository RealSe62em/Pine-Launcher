...
function javaRuntimeArchitectures(processArch, platform = process.platform) {
  if (processArch === 'arm64') return platform === 'win32' ? ['aarch64', 'x64'] : ['aarch64'];
...
function normalizeProfileLoader(profile, loader, loaderVersion) {
  const allowedLoaders = new Set(['vanilla', 'fabric', 'quilt', 'forge', 'neoforge']);
  const presetProfiles = new Set(['performance', 'beginner', 'builder', 'pvp']);
  const normalizedProfile = profile === 'vanilla' || profile === 'custom' || presetProfiles.has(profile) ? profile : 'custom';
  let normalizedLoader = allowedLoaders.has(loader) ? loader : 'vanilla';
  if (normalizedProfile === 'vanilla') normalizedLoader = 'vanilla';
  if (presetProfiles.has(normalizedProfile)) normalizedLoader = 'fabric';
  return {
