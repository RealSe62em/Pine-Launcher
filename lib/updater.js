...
function createUpdateManager({
  ...
  downloadLinuxUpdate = null,
  installLinuxUpdate = null,
  ...
  // Windows uses NSIS. Debian and Arch builds download their native release
  // package, verify it, and hand installation to the system package manager.
  ...
  const automaticInstallSupported = Boolean(isPackaged && (platform === 'win32' ||
    (platform === 'linux' && typeof downloadLinuxUpdate === 'function' && typeof installLinuxUpdate === 'function')));
  let linuxPackage = null;
  let linuxPackagePath = null;
  let started = false;
  let checkTimer = null;
  ...
  async function checkForUpdates({ manual = true } = {}) {
    ...
    if (platform === 'linux') {
      ...
      try {
        ...
        linuxPackage = available ? release.package || null : null;
        if (!available) linuxPackagePath = null;
        return publish(available ? {
          ...
          message: `Pine Launcher ${release.version} is ready to download.`,
        } : {
          status: 'not-available', availableVersion: null, releaseDate: null, releaseNotes: '',
          message: 'You are using the latest version.',
        });
      } catch (error) {
  ...
  async function downloadUpdate() {
    ...
    try {
      if (platform === 'linux') {
        if (!linuxPackage) throw new Error('No compatible Linux package was found for this system.');
        linuxPackagePath = await downloadLinuxUpdate(linuxPackage, progress => publish({
          status: 'downloading',
          percent: clampPercent(progress?.percent),
          transferred: Number(progress?.transferred ?? progress?.bytes) || 0,
          total: Number(progress?.total) || 0,
          bytesPerSecond: Number(progress?.bytesPerSecond) || 0,
          message: `Downloading update… ${Math.round(clampPercent(progress?.percent))}%`,
          installBlocked: false,
        }));
        publish({ status: 'downloaded', percent: 100, message: 'Update downloaded. Restart Pine to install it.' });
      } else {
  ...
  async function installUpdate() {
    ...
    if (platform === 'linux') {
      try {
        await installLinuxUpdate(linuxPackagePath, linuxPackage);
      } catch (error) {
        return reportError(error, 'Update installation failed');
      }
      return snapshot();
    }
