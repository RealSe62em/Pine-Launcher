...
function remoteRecipeFile(relative, remote, url) {
  const source = remote?.hashes || {};
  const hashes = Object.fromEntries(['sha512', 'sha256', 'sha1'].filter(key => source[key]).map(key => [key, String(source[key]).toLowerCase()]));
  if (!Object.keys(hashes).length) throw new Error('Provider verification hash is unavailable');
  return {
    path: String(relative).split('\\').join('/'),
    hashes,
    downloads: [url],
    fileSize: Math.max(0, Number(remote?.size) || 0) || undefined,
  };
}
...
module.exports = { verifiedRecipeFile, remoteRecipeFile, buildLightweightManifest, buildModrinthIndex, hashDescriptor, loaderDependency, modrinthDependencies };
