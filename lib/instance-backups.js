...
const SKIPPED_FULL_ENTRIES = new Set(['logs', 'crash-reports', 'natives', 'quickPlay', '.pine-content-installs']);
...
function validRestoreState(state, allowedRoots) {
  if (!state || !path.isAbsolute(state.target) || !path.isAbsolute(state.staging) || !path.isAbsolute(state.rollback)) return false;
  const target = path.resolve(state.target);
  const parent = path.dirname(target);
  const base = path.basename(target);
  if (path.dirname(path.resolve(state.staging)) !== parent || path.dirname(path.resolve(state.rollback)) !== parent) return false;
  if (!path.basename(state.staging).startsWith(`.${base}.pine-restore-`)) return false;
  if (!path.basename(state.rollback).startsWith(`.${base}.pine-rollback-`)) return false;
  return allowedRoots.some(root => samePath(target, root) || samePath(target, path.join(root, 'saves')));
}

function recoverInterruptedRestores(backupsDir, { allowedRoots = [] } = {}) {
  let entries = [];
  ...
  for (const name of entries) {
    ...
    try {
      ...
      if (!validRestoreState(state, allowedRoots)) throw new Error('Invalid restore marker');
